...
export const ApiNode: React.FC<ApiNodeProps> = ({ data }) => {
  const {
    entity,
    isSelected,
    isConnected,
  ...
  let containerClasses = 'bg-white border-slate-200/90 text-slate-900 shadow-2xs hover:border-teal-400 hover:shadow-md';
  ...
  if (isSelected) {
    containerClasses += ' ring-4 ring-teal-500/25 border-teal-600 shadow-lg scale-[1.02] z-30 opacity-100';
  } else if (isConnected) {
    containerClasses += ' ring-2 ring-teal-400/25 border-teal-400 shadow-sm z-20 opacity-95';
  }
  ...
  return (
    ...
    >
      <Handle
        type="target"
        position={targetPos}
        className="!w-2.5 !h-2.5 !bg-teal-600 !border-2 !border-white"
      />

      {/* Header bar */}
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-slate-100 bg-teal-50/70 rounded-t-xl">
        <div className="flex items-center gap-1.5">
          <div className="p-0.5 rounded bg-teal-100 text-teal-800">
            <Globe className="w-3 h-3" />
          </div>
          <span className="text-[10px] font-bold uppercase tracking-wider text-teal-800">API Gateway</span>
        </div>
      ...
      <div className="px-3 py-2">
        ...
        <span className="text-[10px] text-teal-700/90 font-mono truncate block mt-0.5" title={entity.technology}>
          {entity.technology}
        </span>
        ...
        <div className="mt-2 pt-1.5 border-t border-slate-100 flex items-center justify-between">
          {isCritical ? (
            ...
            <span className="text-[9px] text-teal-700/80 font-mono font-medium">Gateway</span>
          )}
